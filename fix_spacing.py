...
def fix_spacing_in_file(filepath):
    ...
    for line_num, line in enumerate(lines):
        ...
        def process_match_with_context(match):
            ...
            # pattern_start_index, pattern_end_index = pattern_content_span # Not used in final logic, but useful for understanding
            ...
            # This is the character adjacent to the horizontal whitespace matched by [ \t]*
            ...
            if is_macro:
                 ...
                 if param is None:
                      ...
                      # Return the original text matched by the pattern (including original horizontal whitespace)
                ...
                if link_text_match:
                    ...
                    if not is_text_inside_valid:
                         ...
                         pass # Text inside is empty, internal_start/end_char will be None
            ...
            # Logic based on: Adjacent Punctuation => NO space. Adjacent Chi + Internal Adjacent Chi => NO space. Others => ADD space.
            # We check the character immediately BEFORE and AFTER the *full match* (including the horizontal whitespace).
            ...
            if actual_char_before_full_match is not None:
                 # Check if the character is Chinese punctuation
                 is_punct_before = is_chinese_punctuation(actual_char_before_full_match)
                 # Check if the character is a Chinese character (excluding CJK punctuation)
                 is_chinese_before = is_chinese(actual_char_before_full_match)
                 # Check if the character inside the macro/link at the start is Chinese
                 is_internal_start_chinese = is_chinese(internal_start_char) if internal_start_char is not None else False

                 # Add space before if the character is NOT punctuation AND NOT (Chinese char AND Internal start is Chinese)
                 # We assume any other character (English, number, etc.) requires a space if adjacent to CJK.
                 # Also, Non-CJK + Non-CJK generally needs a space.
                 ...
                 pass # No character before (start of line)
            ...
            if actual_char_after_full_match is not None and not is_line_ending_after:
                 ...
                 # Add space after if the character is NOT a line ending, NOT punctuation, AND NOT (Chinese char AND Internal end is Chinese)
                 ...
                 pass # No character after (end of line), or it's a line ending


            # --- Construct the replacement string ---
            # We replace the *full match* (including original leading/trailing horizontal whitespace)
            # with calculated_space + pattern_content + calculated_space
            # Original horizontal spaces matched by [ \t]* are discarded and replaced by our single calculated space or no space.
            # Line endings are NOT matched by [ \t]* and are therefore not part of match.group(0), so they are preserved by re.sub operating line by line.
        ...
        # The pattern now only matches horizontal whitespace, so line endings are preserved.
